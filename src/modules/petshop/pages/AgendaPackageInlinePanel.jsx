...
import { AlertCircle, CheckCircle2, Crown, PackageCheck, Scissors, Search } from 'lucide-react'
...
function ensureInlineRoot(modal) {
  const label = [...(modal?.querySelectorAll?.('label') || [])]
    .find((item) => normalizeAppointmentUiText(item.textContent).startsWith('servicos '))
  const parent = label?.parentElement
  if (!label || !parent) return null

  let root = parent.querySelector(':scope > [data-yuisync-inline-package-root]')
  if (root) return root

  root = document.createElement('div')
  root.setAttribute('data-yuisync-inline-package-root', 'true')
...
function normalizedClientText(client = {}) {
...
function resolveClient(clients = [], visibleText = '') {
  ...
  return (clients || [])
    .map((client) => {
      ...
      const haystack = normalizedClientText(client)
      let score = 0

      if (phone && digits && (digits.includes(phone) || phone.includes(digits))) score += 120
      if (owner && normalized.includes(owner)) score += 80
      if (pet && normalized.includes(pet)) score += 65
      score += owner.split(' ').filter((term) => term.length > 1 && normalized.includes(term)).length * 8
      score += pet.split(' ').filter((term) => term.length > 1 && normalized.includes(term)).length * 10
      if (haystack && normalized && (haystack.includes(normalized) || normalized.includes(haystack))) score += 20
...
function subscriptionForClient(subscriptions = [], clientId = '') {
...
function legacyPackageEntries(usage = [], catalogServices = [], group = 'banho_tosa') {
  const entries = []
  usage
...
export default function AgendaPackageInlinePanel() {
  ...
  const [showTosas, setShowTosas] = useState(false)
  const [tosaSearch, setTosaSearch] = useState('')
  ...
  const refreshData = useCallback(async () => {
    try {
      ...
      console.warn('Falha ao carregar o pacote ativo na Agenda:', error)
      setNotice('Não foi possível carregar o pacote ativo. Atualize a Agenda e tente novamente.')
    }
  ...
  const exactSubscription = useMemo(
    () => subscriptionForClient(subscriptions, resolvedClient?.id),
  ...
  const legacyEntries = useMemo(
    () => legacyPackageEntries(usage, catalogServices, serviceGroupValue),
  ...
  const automaticEntries = useMemo(() => {
    ...
    const unambiguousLegacy = [...legacyByType.values()].filter((items) => items.length === 1).flat()
    return [
      ...realEntries.filter((entry) => Number(entry.remaining || 0) > 0),
      ...unambiguousLegacy,
    ]
  }, [legacyEntries, realEntries])
  ...
  const tosaServices = useMemo(() => {
    const query = normalizeAppointmentUiText(tosaSearch)
  ...
  const syncModal = useCallback(() => {
    ...
    setRoot((current) => {
      const next = ensureInlineRoot(modal)
  ...
  useEffect(() => {
    ...
    const onClick = () => window.setTimeout(scheduleSync, 0)
  ...
  const usePackage = useCallback(async () => {
    if (!automaticEntries.length) {
      setNotice('Este pacote possui mais de um serviço compatível. Escolha abaixo qual será realizado.')
      return
    }
    setBusy(true)
    setNotice('')
  ...
  if (!root || serviceGroupValue !== 'banho_tosa') return null

  return createPortal(
    <div className="mb-3 space-y-3" data-yuisync-inline-package-content>
      {activeSubscription ? (
        <section className="rounded-2xl border border-emerald-400/35 bg-emerald-500/10 p-4 shadow-[0_12px_38px_rgba(16,185,129,0.10)]">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.17em] text-emerald-300"><Crown size={13}/> Pacote ativo · prioridade</p>
              <p className="mt-1 text-base font-black text-text">{packageName(activeSubscription)}</p>
              <p className="mt-1 text-xs text-muted">{activeSubscription.client?.pet_name || resolvedClient?.pet_name || 'Pet'} · Tutor: {activeSubscription.client?.owner_name || resolvedClient?.owner_name || 'Cliente'}</p>
            </div>
            <span className="badge badge-green">Ativo</span>
          </div>

          <div className="mt-3 flex flex-wrap gap-2">
            {usage.map((item) => <span key={item.service_type} className={`badge ${Number(item.remaining || 0) > 0 ? 'badge-blue' : 'badge-gray'}`}>{item.label}: {item.remaining}/{item.total} disponíveis</span>)}
          </div>

          {legacyEntries.length > 0 && <p className="mt-3 rounded-xl border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">Plano legado reconhecido. Os serviços compatíveis abaixo continuarão consumindo o benefício antigo.</p>}

          <button type="button" disabled={busy || !packageEntries.length} onClick={() => void usePackage()} className="btn btn-primary mt-3 w-full justify-center"><PackageCheck size={15}/>{busy ? 'Adicionando...' : `Usar ${packageName(activeSubscription)}`}</button>

          <div className="mt-3 space-y-2">
            {packageEntries.map((entry) => (
              <button key={`${entry.legacy_benefit_type || 'real'}-${entry.service_type}`} type="button" disabled={busy || Number(entry.remaining || 0) <= 0} onClick={() => void chooseService(entry.label)} className="flex w-full items-center gap-3 rounded-xl border border-emerald-500/20 bg-emerald-500/8 px-3 py-2 text-left disabled:cursor-not-allowed disabled:opacity-50">
        ...
        <section className="rounded-xl border border-amber-500/25 bg-amber-500/[0.07] p-3">
          <p className="text-xs font-bold text-text">Nenhum pacote ativo foi vinculado automaticamente a este cliente.</p>
          <p className="mt-1 text-[11px] text-muted">Pesquise abaixo apenas para corrigir um cadastro legado ou vínculo antigo.</p>
          <div className="relative mt-2"><Search size={14} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-muted"/><input className="inp pl-9" value={packageSearch} onChange={(event) => setPackageSearch(event.target.value)} placeholder="Tutor, pet, telefone ou pacote..."/></div>
          {packageSearch && <div className="mt-2 max-h-40 overflow-y-auto rounded-xl border border-[var(--border2)]">{packageMatches.map((subscription) => <button key={subscription.id} type="button" onClick={() => setManualSubscriptionId(subscription.id)} className="w-full border-b border-[var(--border2)] px-3 py-2 text-left last:border-0 hover:bg-white/5"><span className="block text-sm font-bold text-text">{subscription.client?.pet_name || subscription.client?.owner_name}</span><span className="block text-xs text-muted">{subscription.client?.owner_name} · {packageName(subscription)}</span></button>)}{!packageMatches.length && <p className="px-3 py-3 text-center text-xs text-muted">Nenhum pacote ativo encontrado.</p>}</div>}
        </section>
      ) : null}

      <section className="rounded-xl border border-violet-500/20 bg-violet-500/[0.05] p-3">
        <button type="button" onClick={() => setShowTosas((current) => !current)} className="flex w-full items-center justify-between gap-3 text-left">
          <span><span className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.16em] text-violet-300"><Scissors size={13}/> Tosas cadastradas</span><span className="mt-1 block text-xs text-muted">{tosaServices.length} serviço(s) ativo(s) no catálogo</span></span>
          <span className="badge badge-gray">{showTosas ? 'Fechar' : 'Abrir'}</span>
        </button>
        {showTosas && <div className="mt-3"><div className="relative"><Search size={14} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-muted"/><input className="inp pl-9" value={tosaSearch} onChange={(event) => setTosaSearch(event.target.value)} placeholder="Pesquisar qualquer tipo de tosa..."/></div><div className="mt-2 max-h-52 space-y-1 overflow-y-auto">{tosaServices.map((service) => <button key={service.code} type="button" disabled={busy} onClick={() => void chooseService(service.name)} className="flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left hover:bg-violet-500/10"><Scissors size={14} className="shrink-0 text-violet-300"/><span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold text-text">{service.name}</span><span className="block text-xs text-muted">{fmtCurrency(service.default_price || 0)} · {service.default_duration_min || 60} min</span></span></button>)}{!tosaServices.length && <p className="px-3 py-3 text-xs text-muted">Nenhuma tosa ativa encontrada.</p>}</div></div>}
      </section>
