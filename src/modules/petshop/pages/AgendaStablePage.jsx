...
function OperationalEnhancements() {
  ...
  useEffect(() => {
    ...
    const removeGhost = () => dragRef.current?.ghost?.remove()
    ...
    const resetDrag = () => {
      stopAutoScroll()
      removeGhost()
      dragRef.current?.card?.classList.remove('is-pointer-dragging')
    ...
    const compactCard = (card, trigger) => {
      ...
      if (transportBlock) {
        ;[...transportBlock.children].slice(1).forEach((detail) => detail.classList.add('yuisync-card-detail-hidden'))
        transportBlock.style.marginTop = '3px'
      }

      const serviceRow = [...trigger.querySelectorAll('div')].find((element) => (
        String(element.className || '').includes('mt-2')
        && String(element.className || '').includes('justify-between')
      ))
      if (serviceRow) serviceRow.style.marginTop = '4px'
      ...
      card.querySelectorAll('button').forEach((button) => {
        if (button.closest('[data-yuisync-actions]')) return
        const label = normalizeText(`${button.getAttribute('aria-label') || ''} ${button.getAttribute('title') || ''}`)
        if (label.includes('imprimir')) button.classList.add('yuisync-native-print-hidden')
      })
    }

    const actionMarkup = (movable, canComplete) => `
      ${movable ? `<button type="button" data-yuisync-action="drag" class="yuisync-agenda-action yuisync-agenda-drag-handle" aria-label="Arrastar agendamento" title="Arraste para mudar o horario">${ICONS.drag}</button>` : ''}
      <button type="button" data-yuisync-action="print" class="yuisync-agenda-action" aria-label="Imprimir agendamento" title="Imprimir agendamento">${ICONS.print}</button>
      ${canComplete ? `<button type="button" data-yuisync-action="complete" class="yuisync-agenda-action is-complete" aria-label="Concluir agendamento" title="Concluir agendamento">${ICONS.check}</button>` : ''}
    `
    ...
    const syncCards = () => {
      ...
      operationalAppointments.forEach((appointment) => {
        ...
        card.classList.add('yuisync-operational-card')
        ...
        if (outer?.classList.contains('absolute')) outer.classList.add('yuisync-operational-outer')
        ...
        let actions = card.querySelector('[data-yuisync-actions]')
        if (!actions) {
          actions = document.createElement('div')
          actions.dataset.yuisyncActions = 'true'
          actions.className = 'yuisync-agenda-actions'
          card.appendChild(actions)
        }
        const signature = `${movable}:${canComplete}`
    ...
    const onPointerMove = (event) => {
      ...
      if (!state.active) {
        ...
        ghost.querySelectorAll('[data-yuisync-actions]').forEach((node) => node.remove())
    ...
    return () => {
      ...
      resetDrag()
      pageRoot.querySelectorAll('[data-yuisync-actions], [data-yuisync-print-day]').forEach((node) => node.remove())
    }
  }, [completeAppointment, moveAppointment, operationalAppointments, printAppointment, printDay, storeSettings?.petshop_service_durations, transportOptions])
...
export default function AgendaStablePage() {
  return (
    <>
      <AgendaPage />
