...
function ClientPicker({ clients, selectedId, onSelect }) {
  ...
  const results = useMemo(() => clients.filter((client) => clientMatches(client, search)).slice(0, 20), [clients, search])

  return (
    <div className="relative">
      <label className="inp-label">Cliente / pet</label>
      {!open && selected ? (
        <button type="button" onClick={() => setOpen(true)} className="w-full rounded-xl border border-emerald-500/25 bg-emerald-500/8 px-4 py-3 text-left">
        ...
        <button type="button" onClick={() => setOpen(true)} className="btn btn-secondary w-full justify-center"><Search size={14}/> Pesquisar cliente ou pet</button>
      ) : (
          ...
          <div className="mt-2 max-h-64 overflow-y-auto rounded-lg border border-[var(--border2)]">
            {results.map((client) => (
              <button
                ...
                className="flex w-full items-center justify-between gap-3 border-b border-[var(--border2)] px-3 py-2.5 text-left last:border-0 hover:bg-white/5"
              >
                <span className="min-w-0">
                  <span className="block truncate text-sm font-bold text-text">{client.pet_name || 'Pet não informado'}</span>
                  <span className="block truncate text-xs text-muted">{client.owner_name || 'Tutor não informado'}{client.phone ? ` · ${client.phone}` : ''}</span>
                </span>
                {selectedId === client.id && <CheckCircle2 size={15} className="shrink-0 text-emerald-400"/>}
              </button>
            ))}
            {!results.length && <p className="px-3 py-4 text-center text-sm text-muted">Nenhum cliente encontrado.</p>}
          </div>
          <button type="button" onClick={() => { setOpen(false); setSearch('') }} className="btn btn-ghost btn-sm mt-2 w-full justify-center">Fechar busca</button>
        </div>
      )}
    </div>
  )
...
function SubscriptionModal({ plans, clients, catalogServices, onClose, onSave }) {
  ...
  async function submit() {
    if (!form.client_id) return setError('Selecione o cliente e o pet.')
    setSaving(true)
    setError('')
    try {
