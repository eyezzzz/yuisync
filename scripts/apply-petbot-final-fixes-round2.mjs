...
`  return String(message).includes('?')
`,
    'normalização duplicada de pergunta da loja',
  )
...
`,
    'mudança para entrega exige endereço',
  )
...
`  const normalizedMessage = String(item.message || '').normalize('NFD').replace(/[\\u0300-\\u036f]/g, '').toLowerCase()
  ...
  const explicitConfirmation = Boolean(
    ...
    && !/\\b(nao|não|talvez|espera|troca|muda|agora quero|na verdade)\\b/.test(normalizedMessage)
  )
  const explicitCancellation = Boolean(
    item.pendingOrder
    && /\\b(cancelar|cancela|nao quero mais|não quero mais|deixa pra outro dia|descartar)\\b/.test(normalizedMessage)
  )
  const correctsPendingService = Boolean(
    item.pendingOrder?.order?.order_type !== 'produto'
    && /\\b(na verdade|nao e pra|não é pra|troca|muda)\\b/.test(normalizedMessage)
  )
...
console.log('Correções finais round 2 aplicadas com sucesso.')
