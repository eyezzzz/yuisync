...
await writeFile('test/agendaHistoryPrintAction.test.mjs', `import test from 'node:test'\nimport assert from 'node:assert/strict'\nimport { readFile } from 'node:fs/promises'\n\ntest('todo atendimento exibido no historico pode imprimir a ficha', async () => {\n  const source = await readFile(new URL('../src/modules/petshop/pages/AgendaPage.jsx', import.meta.url), 'utf8')\n  const labels = source.match(/aria-label="Imprimir ficha do historico"/g) || []\n  assert.equal(labels.length, 2)\n  assert.equal(source.includes('aria-label="Reimprimir ficha concluida"'), false)\n  assert.doesNotMatch(source, /appt\.status === 'concluido' && \([\\s\\S]{0,260}aria-label="Imprimir ficha do historico"/)\n  assert.doesNotMatch(source, /completed && \([\\s\\S]{0,260}aria-label="Imprimir ficha do historico"/)\n  assert.match(source, /Historico do dia[\\s\\S]*aria-label="Imprimir ficha do historico"[\\s\\S]*<Receipt size=\{13\}\/>/)\n  assert.match(source, /nonBlocking\.map[\\s\\S]*aria-label="Imprimir ficha do historico"[\\s\\S]*<Receipt size=\{11\}\/>/)\n})\n`)

await writeFile('test/teamPendingResponsibleAssignment.test.mjs', `import test from 'node:test'\nimport assert from 'node:assert/strict'\nimport { readFile } from 'node:fs/promises'\n\ntest('equipe permite atribuir apenas servicos concluidos ainda sem responsavel', async () => {\n  const [page, core] = await Promise.all([\n    readFile(new URL('../src/modules/petshop/pages/EquipePage.jsx', import.meta.url), 'utf8'),\n    readFile(new URL('../src/modules/petshop/hooks/usePetshopAdvancedCore.js', import.meta.url), 'utf8'),\n  ])\n  assert.match(page, /assignPendingServiceResponsible/)\n  assert.match(page, /Responsavel manual do servico/)\n  assert.match(page, /Selecionar responsavel/)\n  assert.match(core, /assignPendingServiceResponsible/)\n  assert.match(core, /\.eq\('status', 'concluido'\)/)\n  assert.match(core, /\.is\('responsible_staff_key', null\)/)\n  assert.match(core, /responsible_staff_name: staffName/)\n  assert.match(core, /ja possui responsavel/)\n})\n`)
